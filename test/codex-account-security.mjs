...
import { buildAgentResourceReceipt } from "../src/agent-resource.mjs";
...
test("failed pre-turn thread deletion quarantines telemetry and follow-ups on the same account", async () => {
  class CleanupClient extends FollowupPolicyClient {
    failDelete = false;
    async request(method, params) {
      if (method === "thread/delete" && this.failDelete) throw new Error("SENTRY_DELETE_FAILURE");
      return super.request(method, params);
    }
  }
  const client = new CleanupClient(); let quotaCalls = 0;
  const router = new CodexAccountAgentExecutor({ registry,
    factory: async (account, hooks) => new CodexAgentExecutor({ defaultCwd: cwd, clientFactory: () => client,
      requireChatgptAuth: true, admissionCheck: hooks.assertHealthy, cleanupFailureHandler: hooks.quarantine }),
    quotaProvider: async () => { quotaCalls += 1; return {}; },
  });
  const started = await router.start({ account: "secondary", task: "existing", clientRequestId: "delete-existing" });
  client.accountType = "apiKey"; client.failDelete = true;
  await assert.rejects(router.start({ account: "secondary", task: "rejected", clientRequestId: "delete-rejected" }), { code: "CODEX_AGENT_CLEANUP_FAILED" });
  client.accountType = "chatgpt";
  try {
    await assert.rejects(router.quotaSnapshot({ account: "secondary" }), { code: "CODEX_ACCOUNT_CLEANUP_FAILED" });
    await assert.rejects(router.send({ agentRef: started.agentRef, message: "blocked", clientRequestId: "delete-send", expectedParentTurnId: started.turnId }), /cleanup/i);
    assert.equal(quotaCalls, 0); assert.equal(client.turnCount, 1);
  } finally { await router.close().catch(() => {}); }
});

test("pre-turn cleanup failure blocks an already pending standalone follow-up", async () => {
  class CleanupClient extends FollowupPolicyClient {
    async request(method, params) {
      if (method === "thread/delete") throw new Error("fixture deletion failure");
      return super.request(method, params);
    }
  }
  const client = new CleanupClient();
  const executor = new CodexAgentExecutor({ defaultCwd: cwd, clientFactory: () => client, requireChatgptAuth: true });
  await executor.open();
  const started = await executor.start({ task: "existing", clientRequestId: "direct-cleanup-existing" });
  client.holdMethod = "thread/resume";
  const sending = executor.send({ agentRef: started.agentRef, message: "pending", clientRequestId: "direct-cleanup-send", expectedParentTurnId: started.turnId });
  sending.catch(() => {});
  await client.entered.promise;
  client.accountType = "apiKey";
  await assert.rejects(executor.start({ task: "reject", clientRequestId: "direct-cleanup-rejected" }), { code: "CODEX_AGENT_CLEANUP_FAILED" });
  client.accountType = "chatgpt"; client.release.resolve();
  try {
    await assert.rejects(sending, /cleanup/i);
    assert.equal(client.turnCount, 1);
  } finally { await executor.close().catch(() => {}); }
});

test("queued positive approval rechecks admission while explicit rejection stays available", async () => {
  let onRequest, quarantined = false;
  const client = new PausedClient(), decisions = [];
  const executor = new CodexAgentExecutor({ defaultCwd: cwd,
    clientFactory: options => { onRequest = options.serverRequestHandler; return client; },
    admissionCheck: () => { if (quarantined) throw new Error("fixture account cleanup quarantine"); },
  });
  await executor.open();
  const started = await executor.start({ task: "existing", clientRequestId: "approval-quarantine-start" });
  onRequest({ id: "approval-fixture", method: "item/fileChange/requestApproval", params: { threadId: client.threadId, turnId: started.turnId },
    resolve: result => decisions.push(result), reject: error => decisions.push(error) });
  const approving = executor.resolveApproval({ agentRef: started.agentRef, approvalRequestId: "approval-fixture", clientRequestId: "approval-positive", decision: "approve" });
  quarantined = true;
  try {
    await assert.rejects(approving, /quarantine/);
    assert.deepEqual(decisions, []);
    await executor.resolveApproval({ agentRef: started.agentRef, approvalRequestId: "approval-fixture", clientRequestId: "approval-negative", decision: "reject" });
    assert.deepEqual(decisions, [{ decision: "decline" }]);
  } finally { await executor.close(); }
});

test("server approvals with a mismatched turn cannot bind through a matching thread", async () => {
  let onRequest; const rejected = [];
  const client = new PausedClient();
  const executor = new CodexAgentExecutor({ defaultCwd: cwd, clientFactory: options => { onRequest = options.serverRequestHandler; return client; } });
  await executor.open();
  const started = await executor.start({ task: "existing", clientRequestId: "approval-target-start" });
  try {
    onRequest({ id: "wrong-target", method: "item/fileChange/requestApproval", params: { threadId: client.threadId, turnId: "foreign-turn" },
      reject: error => rejected.push(error), resolve: () => assert.fail("wrong target must not resolve") });
    assert.equal(rejected.length, 1);
    assert.equal((await executor.show({ agentRef: started.agentRef })).pendingApproval, null);
  } finally { await executor.close(); }
});
...
test("legacy migration cannot write while another runtime owns the v2 writer lock", { timeout: 5_000 }, async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "codexless-migration-lock-"));
  try {
    const taskStateFile = path.join(root, "task-state.json");
    const legacy = JSON.stringify({ version: 1, records: [{ taskRef: "old-task", requestId: "old-request", action: "start", updatedAt: Date.now() }] });
    await writeFile(taskStateFile, legacy);
    await writeFile(taskStateDataFile(taskStateFile) + ".lock", JSON.stringify({ pid: process.pid }));
    const state = createAgentPreviewState({ taskStateFile });
    assert.equal(await readFile(taskStateFile, "utf8"), legacy, "migration must not rename a legacy store outside the exclusive lock");
    await assert.rejects(readFile(taskStateDataFile(taskStateFile)), { code: "ENOENT" });
    assert.throws(() => state.taskPersistence.get("old-task"), /locked|safely|unsafe/i);
  } finally { await rm(root, { recursive: true, force: true }); }
});

test("v2 startup blocks conflicting retired migration evidence", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "codexless-migration-conflict-"));
  try {
    const taskStateFile = path.join(root, "task-state.json");
    await mkdir(taskStateFile);
    await writeFile(taskStateDataFile(taskStateFile), JSON.stringify({ version: 2, records: [], requestTombstones: [] }));
    const retired = JSON.stringify({ version: 1, records: [{ taskRef: "old-task", requestId: "old-request", action: "start", updatedAt: Date.now() }] });
    await writeFile(taskStateFile + ".v1-retired", retired);
    const state = createAgentPreviewState({ taskStateFile });
    assert.throws(() => state.taskPersistence.findByRequest({ requestId: "old-request", action: "start" }), /migration|unsafe|safely/i);
    assert.equal(await readFile(taskStateFile + ".v1-retired", "utf8"), retired);
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test("consent and terminal receipts withhold arbitrary quota bucket identifiers and labels", async () => {
  const quota = { status: "ok", observedAt: "2026-09-09T00:00:00.000Z", usage: { status: "ok" },
    rateLimits: { status: "ok", value: { limits: [{ key: "SENTRY_KEY", limitId: "SENTRY_ID", limitName: "SENTRY_NAME",
      planType: "SENTRY_PLAN", rateLimitReachedType: "SENTRY_TYPE", windows: [{ kind: "primary", usedPercent: 23, windowDurationMins: 300 }] }] } } };
  const consent = new MeteredConsentGate({ mode: "always", quotaProvider: async () => quota });
  const prepared = await consent.authorize({ action: "start", requestId: "quota-bucket-leak", payload: { account: "secondary", prompt: "x" } });
  const receipt = buildAgentResourceReceipt({ turnId: "turn-fixture", turnStatus: "completed", quotaSnapshot: quota });
  for (const result of [prepared, receipt]) assert.doesNotMatch(JSON.stringify(result), /SENTRY/);
  assert.equal(prepared.consent.quota.rateLimits.limits[0].windows[0].remainingPercent, 77);
  assert.equal(receipt.accountQuota.rateLimits.limits[0].windows[0].windowDurationMins, 300);
});
